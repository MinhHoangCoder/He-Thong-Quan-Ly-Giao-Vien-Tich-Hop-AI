...
// Dòng của lưới tuần bám khung tiết THẬT trong dữ liệu — GV dạy nhiều trường thì lưới
// lấy theo trường có nhiều tiết nhất (tiểu học 10 tiết).
      ...
      <ul v-if="todaySorted.length" class="timeline">
        <li v-for="t in todaySorted" :key="t.id" class="timeline__item" @click="openLessons(t)">
          ...
          <div class="timeline__body">
            ...
            <small>
              {{ tietLabel(t.periodNumber, t.sessionType, t.indexInSession) }} ·
              {{ t.schoolName }}
          ...
          >
            ...
            <td v-for="d in weekDays" :key="d.iso" class="wcell">
              <button
                ...
                :title="`${tietLabel(e.periodNumber, e.sessionType, e.indexInSession)} · ${e.subjectName} · ${e.schoolName}${e.className ? ' · Lớp ' + e.className : ''} — bấm để mở bài giảng`"
                @click="openLessons(e)"
              >
