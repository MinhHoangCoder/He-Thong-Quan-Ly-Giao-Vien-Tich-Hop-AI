...
|---|---|
| `schema/TSMS_Schema.sql` | Schema lõi (bản thiết kế, để đọc & tham chiếu) |
| `migrations/` | Nơi đặt migration nguồn trước khi đưa vào Flyway (tùy chọn) |
| `seed/` | Script dữ liệu mẫu (roles, branch demo...) |
| `TSMS_TuDien_DB.md` | Từ điển thuật ngữ & hướng dẫn đọc database (tiếng Việt) |

## Flyway (chạy migration tự động)

Migration thực thi nằm trong backend: `backend/src/main/resources/db/migration/`.

- `V1__init_schema.sql` — bản sao của `schema/TSMS_Schema.sql`, Flyway chạy khi backend khởi động.
- Thêm thay đổi schema sau này bằng file mới: `V2__<mô_tả>.sql`, `V3__...` (KHÔNG sửa file đã chạy).

> **Giữ đồng bộ:** `schema/TSMS_Schema.sql` là bản thiết kế để đọc; `db/migration/V1__init_schema.sql` là bản Flyway thực thi. Khi đổi schema, cập nhật cả hai (hoặc coi migration là nguồn chính).
...
- Phần AI (`TSMS_Schema_AI.sql`, 6 bảng) làm ở giai đoạn sau → sẽ thành `V2__ai_schema.sql`.
